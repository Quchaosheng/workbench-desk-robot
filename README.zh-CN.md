...
- 事件库 + 回放
- 五类任务的模板规划器(不需要模型)
- 放置、精确齐套、检验置信度、工作区清障以及按证据推导快递路由的专用验证器
...
快递处理明确限定为桌面入库区内已经放置的包裹:系统先扫描整批,只有标签已核验且外观完好的包裹进入取件架,破损、标签不可读或其他未核验件优先进入异常隔离区。当前设备没有移动底盘、电梯或快递柜访问能力,因此“下楼/去快递柜取件”会失败关闭,不会伪造已取件证据。
