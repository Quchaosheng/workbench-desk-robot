...
def sha256_file(path: Path) -> str:
    """Hash UTF-8 text independently of the platform's line-ending policy.

    The BOM is generated by tools that may use the host platform's default
    newline convention.  Treating CRLF and LF as different bytes would make a
    Windows-generated register fail validation on Linux even though the BOM
    content is identical.  Non-text files are left byte-for-byte hashed so the
    helper remains safe if it is reused for a binary artifact.
    """
    raw = path.read_bytes()
    try:
...
def _is_pristine_pending_row(existing: dict[str, str], expected: dict[str, str]) -> bool:
    """Return whether a stale hash can be safely rebound without an ECO.

    A hash migration is bookkeeping only when every user-owned field still
    matches the generated expectation and no approval identity/evidence has
    been recorded.  Any other difference is treated as a deliberate or
    potentially signed change and must go through an explicit migration.
    """
